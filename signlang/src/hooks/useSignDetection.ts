...
import type { DetectedSign, AppSettings } from '../types';
import { useMediaPipeDetection } from './useMediapipeDetection';

type Language = 'english' | 'spanish' | 'khmer';
...
export const useSignDetection = (settings: AppSettings) => {
  ...
  const translateSign = useCallback((sign: string): string => {
//   const translations = SIGN_TRANSLATIONS[settings.language as Language] || SIGN_TRANSLATIONS.english;
//   if (translations.hasOwnProperty(sign)) {
//     return translations[sign];
//   }
  console.warn(`Translation not found for sign: ${sign}`);
  return sign; // Fallback to original sign
}, [settings.language]);
